    ...
    var PagesCalendar = function(element, options) {
        ...
        */ 
        wView.prototype._buildLayout = function() {
            ...
            $(this.container).html("");
            Calendar.content = '';
            Calendar.content += '<div class="view">';
        ...
        wView.prototype._bindEventDraggers = function() {
                ...
                interact('.event-container:not(.readonly)')
                  .draggable({
                    ...
                    onend: function (event) {
                        ...
                        eventO.start = moment(eventO.start).add(mins, 'minutes');
                        eventO.end = moment(eventO.start).add(el.attr("data-event-duration"), 'minutes');
                        //Update Array
                        Calendar.settings.events[eventData.index] = eventO;
                        mins = 0;
                        days = 0;
                        el.attr("data-starttime",eventO.start.format());
                        el.attr("data-endtime",eventO.end.format());

                        var time = el.children(".event-inner").children(".time-wrap");
                        time.children(".event-start-time").html(moment(eventO.start).format(Calendar.settings.timeFormat));
                        time.children(".event-end-time").html(moment(eventO.end).format(Calendar.settings.timeFormat));
                ...
                .resizable({
                  ...
                  .on('resizeend', function (event) {
                    ...
                    event.end = moment(eventO.start).add(duration,"minutes").format()
                    elem.attr("data-event-duration",duration);
                    elem.attr("data-endtime",event.end);
                    var time = elem.children(".event-inner").children(".time-wrap");
                    time.children(".event-end-time").html(moment(event.end).format(Calendar.settings.timeFormat));
                    //Update Array
                    duration = 0;
                    days = 0;
                    Calendar.settings.events[eventData.index] = eventO;                   
        ...
        */
        ...
        Calendar = {
            ...
            Init: function() {
                ...
                plugin.settings.onViewRenderComplete.call(this);
